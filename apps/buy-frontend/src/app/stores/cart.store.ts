...
const CART_LOCALSTORAGE_KEY = 'buy_cart';
...
export const CartStore = signalStore(
  ...
  withComputed((store) => ({
    ...
    totalAmount: computed(() =>
      store.items().reduce((acc, item) => {
        return acc + item.quantity * item.price; // Fix: multiply instead of add
      }, 0)
    ),
  })),
  withMethods((store) => ({
    ...
    removeFromCart(productId: string) {
      const updateItems = store.items().filter((item) => item.id !== productId);
      patchState(store, { items: updateItems });
